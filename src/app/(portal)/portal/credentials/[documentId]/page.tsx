...
export default async function CredentialDetailPage({ params }: Props) {
  ...
  const credential = result.portfolio.credentials.find(
    (item) => item.id === documentId
  );
  const canDownload = result.portfolio.permissions.canDownloadFiles;
  if (!credential) {
    notFound();
  }
                ...
                >
                  ...
                  <div className="mt-4 flex flex-wrap gap-2 text-sm">
                    <Button asChild variant="outline" className="min-h-10 px-4">
                      <a
                        href={`/api/credentials/files/${file.id}?intent=view`}
                        target="_blank"
                        rel="noreferrer"
                      >
                        {t("detail.openFile")}
                    ...
                    {canDownload ? (
                      <Button asChild variant="light" className="min-h-10 px-4">
                        <a
                          href={`/api/credentials/files/${file.id}?intent=download`}
                        >
