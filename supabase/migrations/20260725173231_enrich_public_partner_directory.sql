-- Enrich the existing public partner directory with business contact and
-- location display fields used by the public detail panel. Email addresses,
-- contact people and all referral/booking rows remain private.

drop function if exists public.get_public_partner_directory(text);

create function public.get_public_partner_directory(
...
comment on function public.get_public_partner_directory(text) is
  'Public partner directory read model. Returns active located partners, public business contact fields, public media and canonical aggregate referral metrics only.';
